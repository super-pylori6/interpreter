...
obj* prim_add(obj* env, obj* list){
  ...
  if(x->type == INT && y->type == INT)
    ...
    if(types[y->tidx].kind == pointer)
	  ...
	  (y->type == GVAR || y->type == RES)){
    if(types[x->tidx].kind == structure &&
       types[y->tidx].kind == structure){
      return make_res(1, x->data + y->data);
    }
    else if(types[x->tidx].kind == pointer &&
	    types[y->tidx].kind == pointer ){
      return make_res(1, x->data + y->data);
    }
    else{
...
void print_gvar(obj* o, int tflag){
  ...
  if(kind == base){
    ...
    print_addr(o->data);
    if(tflag) printf(" :: %s", get_typename(tbit));
    if(tflag && kind == pointer) printf("*");
    else if(tflag && kind == array) printf("[]");
  }
  else if(kind == enumeration){
    print_base(o->data, _UNSIGNED_INT);
    if(tflag) printf(" :: %s", get_typename(tbit));
  }
  else{
    printf("[%s] not defined", __func__);
