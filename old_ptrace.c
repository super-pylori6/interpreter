...
obj* get_base(obj* o){
  ...
  long val = read_mem(8, o->addr);
  return make_res(typenum, val);
}
...
obj* get_pointer(obj* o){
  ...
  if(strcmp(types[typenum].name, "char") == 0){
	...
	if(str[i+j] == '\0'){
	  return make_res(typenum, (long)str);
	}
      }
    } 
  }
  else{
    perror("[get_pointer] not defined");
  }
}

/*
// ポインタの中身をすべて辿って最終的な値を表示する
obj* get_pointer(obj* o){
  int i, j;
  long addr = o->addr;
  long val;
...
obj* get_member_direct(obj* stru, obj* memb){
  ...
  long membaddr = stru->addr + types[typenum].mem[i].offset;
...
obj* prim_member_direct(obj* env, obj* list){
  obj* gvar_struct = lookup(env, list->car);
  if(!gvar_struct){
    perror("[prim_member_direct] no symbol");
    return NULL;
  }
  else if(gvar_struct->cdr->type != GVAR){
    perror("[prim_member_direct] not GVAR");
    return NULL;
  }

  int typenum = gvar_struct->cdr->typenum;
  if(types[typenum].kind != structure){
    perror("[prim_member_direct] not struct");
    return NULL;
  }
  
  return get_member_direct(gvar_struct->cdr, list->cdr->car);
}
...
obj* prim_member_indirect(obj* env, obj* list){
  ...
  return get_member_direct(gvar_struct->cdr, list->cdr->car);
  return get_member_indirect(gvar_struct->cdr, list->cdr->car);
}
...
void print_pointer(obj* o){
  if(strcmp(types[o->typenum].name, "char") == 0){
    printf("%s : %s*", (char*)o->val, types[o->typenum].name);
  }
  else{
    perror("[print_pointer] not defined");
  }
}
...
void print_array(obj* o){
  if(strcmp(types[o->typenum].name, "char") == 0){
    ...
    perror("[print_array] not defined");
  }
}
...
void print(obj* o){
  ...
  case GVAR:{
    //printf("GVAR");
    //obj* var = get_gvar(o);
    //printf("(GVAR) %ld : %s", o->val, types[o->typenum].name);
    printf("GVAR");
    break;
  }
  case RES:
